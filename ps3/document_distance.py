...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    """
    merge_dict = freq_dict1.copy()
    for k,v in freq_dict2.items():
        if k in merge_dict:
            merge_dict[k] += v
        else:
            merge_dict[k] = freq_dict2[k]
    
    return [k for k,v in merge_dict.items() if v == max(merge_dict.values())]
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    tfidf_list = [(k,(v*idf_dict[k])) for k,v in tf_dict.items()]
    return sorted(tfidf_list, key=lambda x:x[1])
